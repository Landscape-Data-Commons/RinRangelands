...
setwd("C:/Users/gharrison/OneDrive - USDA/Documents/R scripts/RinRange_workshop_srm24/Download_1.23.2024/RinRangelands-main (2)/RinRangelands-main/Part1")
```
...
```{r}
climate_data_renamed = climate_data %>% pivot_wider(names_from = "Property", values_from = "Average")
```
  
To convert the data back to a long data set we use the function pivot_longer().  
```{r}
climate_data_long = climate_data_renamed %>% pivot_longer(c("mean annual precipitation", "frost free days", "freeze free days"), names_to = "Property", values_to = "Average")%>% distinct()
```
...
```{r}
all_data = indicator_data %>% inner_join(climate_data_renamed, by = c("corrected_ecosite_id" = "Ecological site legacy ID"))
view(all_data)
```
...
```{r}
all_data = indicator_data %>% mutate(Year = as.factor(year(DateVisited))) %>%
  inner_join(climate_data %>% 
               pivot_wider(names_from = "Property", values_from = "Average"), 
             by = c("corrected_ecosite_id" = "Ecological site legacy ID"))
```
