...
#In ArcGIS we intersected the Sandy Ecological Site group with our data. You can do this  Nelson, could you help me make this intersection work? I think I moved to R because it wasn't going well in R
#nm_data <- sf::st_read("MapLayers/ldc_data_nm_intersect.shp")
nm_data <- data_sf |> subset(State == "NM")

# not sure what the intersect data contains that we dont already have in the data_sf...
nm_data <- nm_data |> dplyr::mutate(NWERN_JER = dplyr::case_when(ProjectKey %in% "NWERN_JER"~ "yes"),
                                    Chihuahuan_desert = "yes",
                                       MLRA42 = dplyr::case_when(mlrarsym %in% "42"~ "yes"),
                                    DateVisited = lubridate::as_date(DateVisited),
                                    Year = lubridate::year(DateVisited) |> as.factor(),
                                    # benchmark points
                                    evaluated = dplyr::case_when(TotalFoliarCover >30 ~ "Meeting",
...
nm_data_long <- as.data.frame(nm_data) |>
  ...
  dplyr::mutate(region = factor(region)|>
                  dplyr::recode( "NWERN_JER" = "Site",
                                 # "SandyESG" = "ESG", # TODO decide if we want SandyESG or not
                                 "MLRA42" = "MLRA",
...
``` {r}
# box plot
ggplot(nm_data_long |> subset(benchmark== "new" &
...
# summarize
nm_data_summary <- nm_data_long |> subset(benchmark== "new" &
                         Year %in% c("2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022") &
                         TotalFoliarCover) |>
