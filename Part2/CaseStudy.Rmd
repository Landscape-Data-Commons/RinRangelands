...
Good start, lets add the Chihuahuan desert study area on this map. 
...
Add a line to indicate the year 2015 on the plot
...
In this section we'll be using the 'zonal_stats/pixel_count/custom_asset' API endpoint. More information: https://docs.climateengine.org/docs/build/html/zonal_statistics.html#rst-zonal-stats-pixel-count-custom-asset
...
spei_df <- read_csv('data/sandy_esg_spei180d_2015_2023.csv')
...
Create new columns for number of pixels and the percent of Sandy ESG in each drought category for each date

```{r}
spei_df_plot <- spei_df_plot |>
  group_by(Date) |>
...
Convert the Category to a factor and filter out the categories that don't represent drought. Drought categories have negative values.
