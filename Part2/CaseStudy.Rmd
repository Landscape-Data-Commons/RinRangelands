...
First lets make a map of the region of the boundaries of the chihuahuan desert within the US
```{r}
# Gather state boundaries using rnaturalearth package
us_states <- rnaturalearth::ne_states(country = "united states of america", returnclass = "sf")

# make a map using ggplot() with the geom_sf() function 
us_state_map <- ggplot()+ 
  geom_sf(data=us_states)
#display the map 
us_state_map

# this looks okay... we do not need alaska or Hawaii. Lets remove those states 
# also change the mapping color aesetics 
us_state_map_base_map <- ggplot()+
  geom_sf(data=us_states %>%  filter(!name %in% c("Alaska", "Hawaii")),
          fill = "white", color = "black")+
  theme_minimal()
us_state_map_base_map # looking better already 

# lets add in the boundary for the Chihuahan desert 
# add this on to our prior map
us_state_map <- us_state_map_base_map +
  geom_sf(data = chihuahuan_desert, aes(fill = "Chihuahuan Desert"))
us_state_map

# that seems find but I want the power to customize the color palette
us_state_map <- us_state_map_base_map +
...
## 3. Apply benchmark
Start by removing the training benchmark data, and then score the plots based on if they are meeting or not meeting benchmarks
``` {r}
...
Convert these data to spatial objects
...
Lets make another map with these points!
```{r}
# recall study area map
us_state_map


# this map is great for providing context of our study are in the US.... but to see the plot points on a map we are going to need to zoom into New Mexico and AZ
zoomed_state_map <- ggplot()+
  geom_sf(data=us_states %>%  filter(name %in% c("New Mexico", "Arizona")),
          fill = "white", color = "black")+
  theme_minimal()
zoomed_state_map
  
  
## add in the Chihuahan Desert and Sandy ESG map layers
...
consider changing dot size using size = function
```{r}
...
Now we can see the plot locations, but I wonder where plots are that are meeting or not meeting th benchmark? 
We can use the 'evaluated' column to change the dot color
```{r}
...
nm_data$SandyESG <- sf::st_within(nm_data, sandy_esg_transformed)

head(summary(nm_data$SandyESG), 20) # the values in this col are either 0 or 1

# Convert the logical values to "Yes" or "No" if needed
nm_data$SandyESG <- ifelse(nm_data$SandyESG, "yes", "No")
head(summary(nm_data$SandyESG), 20)
...
nm_data_long$region <- factor(nm_data_long$region,
                              levels = c( "Site", "ESG","MLRA","Ecoregion"))
```
...
# add a line for 2015, which is the focus of our case study 
rap_cover<- rap_cover+ 
